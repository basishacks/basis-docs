...
createdb basis_auth
DATABASE_URL="postgresql://$USER@localhost:5432/basis_auth" bun run db:migrate
```
...
```mermaid
flowchart LR
  S[Edit schema.ts] --> G[bun run db:generate]
  G --> R[Review SQL]
  R --> M[bun run db:migrate]
```

`bun run db:generate` produces a new SQL migration from `src/database/schema.ts`;
review it, then apply with `bun run db:migrate`. Startup also applies pending
